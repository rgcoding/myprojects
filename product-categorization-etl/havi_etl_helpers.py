...
class AWSLambdaClient:
    def __init__(self, region='us-west-2', env='int'):
...
class HAVILambdaClient(AWSLambdaClient):
    ...
    def get_location_by_id(self, location_id):
        ...
        body = self.get_body('havi-lab-api-service-{env}-get'.format(env=self.env), location_by_id_payload)
        location = None
        if body:
    ...
    def get_next_location(self, location_type, process_type, etl_type):
        ...
        body = self.get_body('havi-lab-api-service-{env}-post_put'.format(env=self.env), next_location_payload)
        data = None
        if body:
            data = body.get('data', None)
        return data
    ...
    def update_location(self, location):
        ...
        body = self.get_body('havi-lab-api-service-{env}-post_put'.format(env=self.env), update_location_payload)
        data = None
        if body:
            data = body.get('data', None)
        return data

    def get_categorization(self, request):
        ...
        body = self.get_body('havi-lab-api-service-{env}-categorization'.format(env=self.env), categorization_payload)
        data = None
        if body:
            data = body.get('data', None)
        return data
    ...
    def get_store_by_id(self, store_id):
        ...
        body = self.get_body('havi-lab-api-service-{env}-get'.format(env=self.env), store_id_payload)
        location = None
        if body:
            data = body.get('data', None)
            if data:
    ...
    def get_inventory_chunk(self, store_id, view, chunk_size, offset):
        ...
        body = self.get_body('havi-lab-api-service-{env}-get'.format(env=self.env), inventory_chunk_payload)
        inventory_list = None
        total_rows = 0
        if body:
            data = body.get('data', None)
    ...
    def create_etl_log(self, etl_log):
        ...
        try:
            body = self.get_body('havi-lab-api-service-{env}-post_put'.format(env=self.env), etl_log_payload)
            etl_log_list = list()
    ...
    def update_inventory(self, http_method, inventory):
        ...
        try:
            body = self.get_body('havi-lab-api-service-{env}-post_put'.format(env=self.env), inventory_chunk_payload)
            if body:
                data = body.get('data', None)
                if data:
                    return data
    ...
    def delete_inventory(self, inventory_ids):
        ...
        try:
            body = self.get_body('havi-lab-api-service-{env}-delete'.format(env=self.env), inventory_payload)
            if body:
                data = body.get('data', None)
                if data:
                    return data
